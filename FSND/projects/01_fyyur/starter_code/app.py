...
def show_venue(venue_id):
  ...
  """
  venue = Venue.query.filter_by(id=venue_id).order_by('id').all()[0]
  # TODO: update date filter
  upcoming_shows_count = len(venue.shows)
  venue.upcoming_shows_count = upcoming_shows_count

  if upcoming_shows_count > 0:
    edited_shows = []
    for show in venue.shows:
      show.artist_image_link = Artist.query.filter_by(id=show.artist_id).order_by('id').all()[0].image_link
      edited_shows.append(show)

  return render_template('pages/show_venue.html', venue=venue, venue_shows=edited_shows)
